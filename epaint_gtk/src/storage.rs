...
type PathCallback = Box<dyn Fn(&Path) -> epaint::Result<Vec<u8>>>;
type ResetCallback = Box<dyn Fn() -> epaint::Result<Vec<u8>>>;
...
impl StorageManager {
    pub fn connect_load<F: Fn(&Path) -> epaint::Result<Vec<u8>> + 'static>(&self, callback: F) {
        *self.load_callback.borrow_mut() = Box::new(callback);
    }

    pub fn connect_save<F: Fn(&Path) -> epaint::Result<Vec<u8>> + 'static>(&self, callback: F) {
        *self.save_callback.borrow_mut() = Box::new(callback);
    }

    pub fn connect_reset<F: Fn() -> epaint::Result<Vec<u8>> + 'static>(&self, callback: F) {
...
impl StorageManagerBuilder {
    ...
    pub fn build(self) -> Rc<StorageManager> {
        let storage_manager = Rc::new(StorageManager {
            ...
            reset_callback: RefCell::new(Box::new(|| Err(epaint::Error::NotImplemented))),
            save_callback: RefCell::new(Box::new(|_| Err(epaint::Error::NotImplemented))),
            load_callback: RefCell::new(Box::new(|_| Err(epaint::Error::NotImplemented))),
